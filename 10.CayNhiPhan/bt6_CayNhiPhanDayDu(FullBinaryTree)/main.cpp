...
/*
	- Cây nhị phân được gọi là đầy đủ nếu mọi node trung gian của nó đều có 2 con trái và phải. Nhiệm vụ của bạn là kiểm tra 
	  xem cây nhị phân đã cho có phải là cây nhị phân đầy đủ hay không, có in ra YES, ngược lại in ra NO.
...
bool check(node *root) {
	...
	else if (root->left != NULL && root->right != NULL) return check(root->left) && check(root->right);
	else return false;
}
